...
#include "UnityCG.cginc"
	...
	fixed4 frag (v2f i) : SV_Target
	{
		...
		half lightColor = (dot (worldNormal, half3(0,1,0)) + 1) ;
		...
		fixed4 grabColor = tex2D (_GrabTexture, screenUV + norm.rg * .02);
		...
		//return half4(lerp (grabColor.rgb, col.rgb, fresnel), .5) +pow (fresnel, 1);
		return half4(col.rgb, _Alpha + .6) + pow (fresnel, 2);
		//return half4(grabColor.rgb, .5) + pow (fresnel, 1);
	}
		ENDCG
	}
	}
